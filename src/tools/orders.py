...
def place_order(buyer_chat_id: str, store_identifier: str, item: str):
    ...
    cur.execute(
        "INSERT INTO order_status (buyer_chat_id, store_chat_id, store_name, item, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (
            str(buyer_chat_id),
            str(store_chat_id),
            store_identifier,
...
def handle_store_callback(callback_data: str, store_chat_id: str):
    ...
    if action == "accept":
        ...
        payload = {
            ...
            "text": f"⚠️ *{store_name}* reports *{item}* is out of stock.\nWould you like to skip or chat with the store?",
            "parse_mode": "Markdown",
            "reply_markup": json.dumps({
                "inline_keyboard": [
                    [
